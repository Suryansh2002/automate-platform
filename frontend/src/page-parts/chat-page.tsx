...
export default function ChatPage() {
  ...
  const handleUnsetToken = async () => {
    const sessionId = localStorage.getItem("session_id") || "";
    try {
      await fetch(`${backendUrl}/unset-token/`, {
        method: 'POST',
        headers: {
      ...
      console.error('Error unsetting token:', error);
    }
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      <div className="bg-blue-600 text-white p-4 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleUnsetToken}
            className="px-4 py-2 bg-red-400 hover:bg-red-500 rounded-lg text-sm"
          >
            Logout
          </button>
        </div>
      ...
      <div className="bg-white p-4 border-t">
        <form onSubmit={handleSend} className="flex gap-2 md:px-24">
          <input
            type="text"
            value={input}
